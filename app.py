...
def insert_medication(patient_id: str, med_data: dict, file_info: dict = None) -> bool:
    try:
        with get_connection() as conn:
            with conn.cursor() as cur:
                # نستخدم NULL إذا لم يرفع المستخدم ملفاً
                f_name = f"'{file_info['name']}'" if file_info else "NULL"
                f_path = f"'{file_info['path']}'" if file_info else "NULL"
                f_type = f"'{file_info['type']}'" if file_info else "NULL"
                
                cur.execute(f"""
                    INSERT INTO workspace.healthcare_platform.medications (
                        med_id, patient_id, drug_name, dosage_mg, frequency, 
                        prescribing_doc, prescribed_at, file_name, file_path, file_type
                    ) VALUES (
                        '{str(uuid.uuid4())[:8]}', '{patient_id}', '{med_data['drug_name']}',
                        {med_data['dosage_mg']}, '{med_data['frequency']}', '{med_data['prescribing_doc']}',
                        '{med_data['prescribed_at']}', {f_name}, {f_path}, {f_type}
                    )
                """)
        return True
    except Exception as e:
    ...
    with tab_meds:
        ...
        if df_docs.empty:
            ...
            with st.form("prescribe_med_form", clear_on_submit=True):
                ...
                # إضافة حقل رفع الملف هنا
                uploaded_presc = st.file_uploader("Upload Prescription Image (Optional)", type=['jpg', 'jpeg', 'png'])
            ...
            if submit_med:
                if not drug_name.strip():
                    ...
                    # 1. تجهيز بيانات الملف (إذا تم رفعه)
                    file_info = None
                    if uploaded_presc:
                        save_path = f"/Volumes/workspace/healthcare_platform/my_model_storage/{uploaded_presc.name}"
                        # حفظ الملف في الـ Volume
                        with open(save_path, "wb") as f:
                            f.write(uploaded_presc.getbuffer())
                        
                        file_info = {
                            "name": uploaded_presc.name,
                            "path": save_path,
                            "type": uploaded_presc.type
                        }

                    # 2. تجهيز بيانات الدواء
                    med_data = {
                    ...
                    # 3. الحفظ (مررنا file_info الجديد للدالة)
                    with st.spinner("Saving prescription..."):
                        success = insert_medication(selected_patient_id, med_data, file_info)
